ff2xSRC = \
./FF2X/ff2x_main.f90 ./FF2X/ff2x.f90 ./PAR/parallel.f90 \
./COMMON/wlog.f90 ./COMMON/str.f90 ./COMMON/chopen.f90 \
./FF2X/reff2x.f90 ./COMMON/rdhead.f90 ./COMMON/setkap.f90 \
./FF2X/ff2xmujas.f90 ./COMMON/padlib.f90 ./FF2X/rdfbinl.f90 \
./COMMON/str2dp.f90 ./FF2X/ff2gen.f90 ./DEBYE/sigms.f90 \
./MATH/dist.f90 ./COMMON/pertab.f90 ./DEBYE/sigrem.f90 \
./COMMON/itoken.f90 ./DEBYE/sigcl.f90 ./DEBYE/sigte3.f90 \
./DEBYE/sigm3.f90 ./COMMON/pijump.f90 ./MATH/terp.f90 \
./MATH/polint.f90 ./FF2X/feffdt.f90 ./FF2X/exconv.f90 \
./FF2X/xscorr.f90 ./MATH/terpc.f90 ./MATH/conv.f90 \
./FF2X/ff2afsjas.f90 ./FF2X/dwaddl.f90 ./FF2X/fprime.f90 \
./FF2X/ff2chijas.f90 ./COMMON/getxk.f90 ./FF2X/ff2xmu.f90 \
./FF2X/rdfbin.f90 ./FF2X/ff2afs.f90 ./FF2X/ff2chi.f90 
ff2x_MODULESRC = \
./PAR/m_par.f90 ./COMMON/m_constants.f90 ./COMMON/m_dimsmod.f90 \
./KSPACE/m_controls.f90 ./KSPACE/m_struct.f90 ./KSPACE/m_boundaries.f90 \
./KSPACE/m_kklist.f90 ./KSPACE/m_strfacs.f90 ./COMMON/m_inpmodules.f90 \
./COMMON/m_nrixs.f90 ./COMMON/m_kinds.f90 ./DMDW/m_const_and_conv.f90 \
./INPGEN/m_strings.f90 ./INPGEN/m_ptable.f90 ./DMDW/m_math.f90 \
./DMDW/m_dmdw.f90 ./FF2X/m_thermal_xscorr.f90 ./COMMON/m_feff_results.f90 \
./ERRORMODS/m_errorfile.f90 
